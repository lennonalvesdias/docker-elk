...
poll_ready elasticsearch 'http://localhost:9200/' 'elastic:lead4857'

log 'Waiting for Kibana readiness'
poll_ready kibana 'http://localhost:5601/api/status' 'kibana:lead4857'
...
response="$(curl 'http://localhost:5601/api/saved_objects/_find?type=index-pattern' -u elastic:lead4857)"
echo "$response"
...
sleep 1
curl -X POST 'http://localhost:9200/_refresh' -u elastic:lead4857 \
	-s -w '\n'

log 'Searching message in Elasticsearch'
response="$(curl 'http://localhost:9200/_count?q=message:dockerelk&pretty' -u elastic:lead4857)"
echo "$response"
